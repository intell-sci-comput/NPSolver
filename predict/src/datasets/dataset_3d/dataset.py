...
class Fourier3DGenerator:
    def __init__(self, M, dim=2, norm='max'):
        self.dim = dim
        self.M = M
        self.norm = norm

    def get_fourier_init_condition(self, cells_centroid, norm='max'):
        M = self.M
        xx = cells_centroid[:, 0]
        yy = cells_centroid[:, 1]
        zz = cells_centroid[:, 2]
        Wx = 0.
        Ax = torch.normal(0., 1., size=(M, M, M)).to(device=cells_centroid.device)
        Bx = torch.normal(0., 1., size=(M, M, M)).to(device=cells_centroid.device)
        cxy = torch.normal(-1., 1., size=(2,)).to(device=cells_centroid.device)
        for i in range(M):
            for j in range(M):
                for k in range(M):
                    phase = (i - M // 2) * xx + (j - M // 2) * yy + (k - M // 2) * zz
                    Wx = Wx + Ax[i, j, k] * torch.sin(phase) + Bx[i, j, k] * torch.cos(phase)

        Ux = Wx + cxy[0]
        if norm == 'max':
            Ux = Ux / (Ux.abs().max() + 1e-8)
        elif norm == 'gauss':
            Ux = (Ux - Ux.mean()) / (Ux.std() + 1e-8)
        else:
            raise NotImplementedError(f'Normalization {norm} not implemented.')

        return Ux.unsqueeze(-1)  # [num_cells, 1]
    
    def __call__(self, pos):
        return self.get_fourier_init_condition(pos, self.norm)


class Train3DDataset(Dataset):
    def __init__(self, num_samples, mesh_file, M=10, dim=3, norm_fourier='max', norm_f=False):
        super().__init__()
        self.dim = dim
        self.f_sampler = Fourier3DGenerator(M=M, dim=dim, norm=norm_fourier)
...
def my_collate_fn(batch):
    ...
    for b_i, (pos, f_sample, pos_bd, mesh_info) in enumerate(batch):
        ...
        bc_type_bd = get_bc(mesh_info)
        bc_type_bd_pad[b_i, :n_bd] = bc_type_bd
        mesh_list.append(mesh_info)
    batch_func = BatchMesh(mesh_list)
    batched_mesh = batch_func.batch_mesh()
    x_bd_pad = torch.cat((pos_bd_pad, bc_type_bd_pad), dim=-1)  # (b_size, n_bd_max, pos_dim + bd_size)
...
class Test3DDataset(Dataset):
    def __init__(self, data_dir, file_names, dim=3, apply_bc=False):
            ...
            with h5py.File(osp.join(self.data_dir, file_name), 'r') as file:
                for key in file.keys():
                    ...
                    mesh_info = Mesh(
                        ...
                        patch2mesh=torch.tensor(g['patch2mesh'][:], dtype=torch.long),
                        cell_offset=torch.tensor(g['cell_offset'][:], dtype=torch.long),
                        case_name=g['case_name'][()].decode("utf-8")
                    )
