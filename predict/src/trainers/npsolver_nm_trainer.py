...
class NPSolverNMTrainer:
    ...
    def _train_loop(self, tr_loader, epoch):
        ...
        for batch in (pbar := tqdm(tr_loader)):
            ...
            ref_value = p_predicted[:, 0:1, :]
            p_predicted = p_predicted - ref_value  # (b, n_max, 1)
    ...
    def test(self, dataset):
        ...
        for i in tqdm(range(length)):
            ...
            mesh = mesh.to(self.device)  # if you want to measure the memory usage, comment this line
            y = y.to(self.device)
            start = time.time()
            p_pred = self.model(x, f, x_bd).squeeze(dim=0)  # (num_cells, 1)
            ref_value = p_pred[0:1, :]
            p_pred = p_pred - ref_value  # (b, n_max, 1)
            end = time.time()
            f = f.squeeze(dim=0)
            r = self.phy_loss_func(p_outputs=p_pred, f_samples=f, mesh=mesh)  # if you want to measure the memory usage, comment this line
            # r = torch.tensor(0)  # if you want to measure the memory usage, activate this line
            rmse = rmse_func(p_pred, y)
            rel_l2 = rel_l2_func(p_pred, y)
            peak_alloc = torch.cuda.max_memory_allocated(self.device) / (1024 ** 2)  # MB
            ...
            memory_dict[category].append(peak_alloc)
        ...
        for category in p_pred_dict.keys():
            ...
            mean_memory_dict[category] = np.mean(memory_dict[category])
        return x_dict, f_dict, p_pred_dict, y_dict, mean_r_dict, mean_time_dict, mean_rmse_dict, mean_rel_l2_dict, mean_memory_dict
