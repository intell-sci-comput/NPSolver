...
class Transolver_block(nn.Module):
    ...
    def __init__(
        ...
        self.fx_ln_1 = nn.LayerNorm(hidden_dim)
        self.fx_bd_ln_1 = nn.LayerNorm(hidden_dim)
        self.Attn = Physics_Attention_Irregular_Mesh(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads,
                                                     dropout=dropout, slice_num=slice_num)
        self.fx_ln_2 = nn.LayerNorm(hidden_dim)
        self.mlp = MLP(hidden_dim, hidden_dim * mlp_ratio, hidden_dim, n_layers=0, res=False, act=act)
        if self.last_layer:
            self.fx_ln_3 = nn.LayerNorm(hidden_dim)
            self.mlp2 = nn.Linear(hidden_dim, out_dim)

    def forward(self, fx, fx_bd, mask_inter, mask_bdry):
        fx = self.fx_ln_1(fx)
        fx_bd = self.fx_bd_ln_1(fx_bd)
        fx = self.Attn(fx, fx_bd, mask_inter, mask_bdry) + fx
        fx = self.mlp(self.fx_ln_2(fx)) + fx
        if self.last_layer:
            return self.mlp2(self.fx_ln_3(fx))
        else:
            return fx
...
class Model(nn.Module):
    def __init__(self,
        ...
        if self.unified_pos:
            ...
            self.preprocess_bd = MLP(bdry_dim, n_hidden * 2, n_hidden, n_layers=0, res=False, act=act)
        if Time_Input:
    ...
    def forward(self, x, fx, x_bd, mask=None, mask_bd=None, T=None):
        if self.unified_pos:
            x = self.get_grid(x, x.shape[0])
        if fx is not None:
            fx = torch.cat((x, fx), -1)
            fx = self.preprocess(fx)
            fx_bd = self.preprocess_bd(x_bd)
        else:
        ...
        for block in self.blocks:
            fx = block(fx, fx_bd, mask_inter=mask, mask_bdry=mask_bd)
